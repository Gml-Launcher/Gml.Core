...
internal class Program
{
    ...
    private static async Task CreateProfile(IGmlManager gmlManager)
    {
        ...
        var profileName = System.Console.ReadLine();
        ...
        var gameVersion = System.Console.ReadLine();
        ...
        var loaderChoice = System.Console.ReadLine();
        ...
        if (loader != GameLoader.Vanilla)
        {
            ...
            foreach (var version in versions) System.Console.WriteLine($"{index++}. {version}");

            System.Console.Write("Выберите номер версии: ");
            var versionChoice = System.Console.ReadLine();

            if (int.TryParse(versionChoice, out var versionIndex) && versionIndex > 0 &&
    ...
    private static async Task DeleteProfile(IGmlManager gmlManager)
    {
        ...
        if (int.TryParse(input, out var profileIndex) && profileIndex > 0 && profileIndex <= profiles.Count())
        {
            ...
            if (!versions.Any())
            {
                System.Console.WriteLine($"Нет доступных версий для {loader}" +
                                         (string.IsNullOrWhiteSpace(gameVersion)
                                             ? ""
                                             : $" с версией Minecraft {gameVersion}"));
                return;
            }
    ...
    private static async Task ShowUsers(IGmlManager gmlManager)
    {
        ...
        foreach (var user in users) System.Console.WriteLine($"{index++}. {user.Name} (UUID: {user.Uuid})");
    }
    ...
    private static async Task AuthenticateUser(IGmlManager gmlManager)
    {
        ...
        foreach (var service in services) System.Console.WriteLine($"{index++}. {service.Name} ({service.AuthType})");

        System.Console.Write("Выберите сервис аутентификации: ");
        var serviceChoice = System.Console.ReadLine();

        if (int.TryParse(serviceChoice, out var serviceIndex) && serviceIndex > 0 && serviceIndex <= services.Count())
        {
    ...
    private static async Task AddServerToProfile(IGmlManager gmlManager)
    {
        ...
        var serverName = System.Console.ReadLine();
        ...
        var serverAddress = System.Console.ReadLine();
        ...
        var portInput = System.Console.ReadLine();
    ...
    private static async Task RemoveServerFromProfile(IGmlManager gmlManager)
    {
        ...
        if (int.TryParse(input, out var serverIndex) && serverIndex > 0 && serverIndex <= servers.Count())
        {
    ...
    private static async Task FindMods(IGmlManager gmlManager)
    {
        ...
        System.Console.Write("Ваш выбор: ");
        var modTypeChoice = System.Console.ReadLine();
        ...
        var searchQuery = System.Console.ReadLine();
        ...
        try
        {
            ...
            foreach (var mod in mods)
                System.Console.WriteLine(
                    $"{index++}. {mod.Name} - {mod.Description.Substring(0, Math.Min(mod.Description.Length, 100))}...");
        }
        catch (Exception ex)
        {
    ...
    private static async Task ShowModsForProfile(IGmlManager gmlManager)
    {
        ...
        try
        {
            ...
            foreach (var mod in mods) System.Console.WriteLine($"{index++}. {mod.Name} - {mod.Type}");
        }
        catch (Exception ex)
        {
