...
{
    public async Task<IProfileServer> AddMinecraftServer(IGameProfile profile, string serverName, string address,
        int port)
    {
    ...
    public async Task RemoveServer(IGameProfile profile, string serverName)
    {
        ...
        if (server is not null) profile.Servers.Remove(server);
