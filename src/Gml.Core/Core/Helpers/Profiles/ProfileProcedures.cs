...
public partial class ProfileProcedures : IProfileProcedures
{
    ...
    private readonly ConcurrentDictionary<string, IEnumerable<string>> _quiltVersions = new();
        ...
        IUser user)
    {
        ...
        if (process != null)
            return new GameProfileInfo
            {
                ...
                ReleativePath = profile.ReleativePath,
                ...
                try
                {
                    ...
                    switch (_launcherInfo.StorageSettings.StorageType)
                    {
                        ...
                        case StorageType.S3:
                            ...
                            if (!await _gmlManager.Files.CheckFileExists("profiles", file.Hash))
        ...
        try
        {
            ...
            switch (gameLoader)
            {
                ...
                case GameLoader.NeoForge:
                    ...
                    if (_neoForgeVersions is null || string.IsNullOrEmpty(minecraftVersion)) return [];
    ...
    {
        foreach (var folder in folders) RemoveWhiteListFolderIfNotExists(profile, folder);
    ...
    {
        foreach (var folder in folders) AddWhiteListFolderIfNotExists(profile, folder);
    ...
    {
        foreach (var file in files) AddWhiteListFileIfNotExists(profile, file);
    ...
    private static void AddWhiteListFileIfNotExists(IGameProfile profile, IFileInfo file)
    {
        ...
        if (!profile.FileWhiteList.Contains(file)) profile.FileWhiteList.Add(file);
    }
    ...
    private void RemoveWhiteListFolderIfNotExists(IGameProfile profile, IFolderInfo folder)
    {
        ...
        if (profile.FolderWhiteList.Contains(folder)) profile.FolderWhiteList.Remove(folder);
    }
