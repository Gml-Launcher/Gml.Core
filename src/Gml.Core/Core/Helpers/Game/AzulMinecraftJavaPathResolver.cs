...
public class AzulMinecraftJavaPathResolver : IJavaPathResolver
{
    ...
    public AzulMinecraftJavaPathResolver(MinecraftPath path)
    {
        this._path = path;
    }
    ...
    public string GetJavaBinaryPath(JavaVersion javaVersionName, RulesEvaluatorContext rules)
    {
        ...
        runtime = Directory.GetDirectories(runtime).First();
        return (rules.OS.Name) switch
        {
            ...
            LauncherOSRule.OSX => Directory.GetFiles(runtime, "java", SearchOption.AllDirectories).First(),
    ...
    private string getJavaDirPathForOS(RulesEvaluatorContext rules) =>
        Path.Combine(
            _path.Runtime,
            AzulMinecraftJavaManifestResolver.GetOSNameForAzulArchiveJava(rules.OS));
}
